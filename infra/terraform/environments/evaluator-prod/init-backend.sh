...
if [[ -z "${AWS_REGION}" || -z "${STATE_BUCKET}" || -z "${STATE_KMS_KEY_ARN}" ]]; then
  echo "ERROR: Missing required backend values (aws_region, terraform_state_bucket, terraform_state_kms_key_arn) in terraform.tfvars." >&2
  exit 1
fi
...
STATE_KEY="${TERRAFORM_STATE_KEY:-evaluator/prod/terraform.tfstate}"
