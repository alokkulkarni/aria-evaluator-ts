...
# ── Cross-VPC peering metadata (consumed by connectivity-prod) ─────────────────
# Lets connectivity-prod peer this evaluator VPC with the control-plane VPC so the
# evaluator can reach the internal control-plane ALB for the SSO verify call.

output "vpc_cidr" {
  description = "Evaluator VPC CIDR (used in peer route tables)"
  value       = module.networking.vpc_cidr
}

output "all_route_table_ids" {
  description = "All evaluator route table IDs — peer stacks add reverse routes to each"
  value       = module.networking.all_route_table_ids
}

output "ecs_service_security_group_id" {
  description = "Evaluator ECS task SG — peer stacks reference it for control-plane ALB ingress"
